...
async fn main() -> Result<()> {
    ...
    //
    /*
    let token_data_store1 = Arc::new(LockFreeTokenDataStore::new());

    let token_data_store_clone = Arc::clone(&token_data_store1);
    tokio::spawn(async move {
        token_data_store_clone.log_token_pair_stats().await;
    });
    */
    //
    // Тестовое логирование для проверки
        ...
        loop {
            ...
            match pinger.ping(PingSequence(seq), &[]).await {
                Ok((IcmpPacket::V4(_packet), rtt)) => {
                    ...
                    if latency_ms > 50 {
                        warn!("High latency detected: {}ms", latency_ms);
                ...
                Ok((IcmpPacket::V6(_), _)) => {
                    ...
                    warn!("Received unexpected ICMPv6 packet.");
                    consecutive_failures += 1;
                }
