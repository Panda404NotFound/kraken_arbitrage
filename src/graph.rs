...
impl ArbitrageEngine {
    ...
    async fn find_unused_pairs(&self) -> Vec<String> {
        ...
        // Находим неиспользуемые пары и форматируем их
        let unused_pairs: Vec<String> = all_pairs
            .difference(&used_pairs)
            .map(|pair| self.format_pair(pair))
            .collect();
    ...
    fn format_pair(&self, pair: &str) -> String {
        let quote_currencies = &self.config.quote_currencies;
        
        for (i, quote) in quote_currencies.iter().enumerate() {
            if pair.ends_with(quote) {
                let base = &pair[..pair.len() - quote.len()];
                return format!("{}/{}", base, quote);
            }
            if pair.starts_with(quote) {
                let base = &pair[quote.len()..];
                return format!("{}/{}", quote, base);
            }
            if i < quote_currencies.len() - 1 {
                let next_quote = &quote_currencies[i + 1];
                if pair.starts_with(quote) && pair.ends_with(next_quote) {
                    let base = &pair[quote.len()..pair.len() - next_quote.len()];
                    return format!("{}/{}/{}", quote, base, next_quote);
                }
            }
        }
        
        pair.to_string() // Возвращаем исходную строку, если не удалось отформатировать
    }
