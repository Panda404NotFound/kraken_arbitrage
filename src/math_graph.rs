...
impl MathGraph {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        loop {
            ...
            if let Err(e) = self.calculation_cycle().await {
                error!("Ошибка в цикле вычислений: {:?}", e);
            }
    ...
    async fn calculation_cycle(&self) -> Result<()> {
        ...
        for chain in local_chains {
            if let Err(e) = tx.send(chain).await {
                error!("Ошибка при отправке цепочки в канал: {:?}", e);
            }
        }
        ...
        for worker in workers {
            if let Err(e) = worker.await {
                error!("Ошибка в рабочей задаче: {:?}", e);
            }
        }
    ...
    async fn process_chain_result(&self, result: Result<ChainResult>) {
        match result {
            ...
            Err(e) => error!("Ошибка при расчете прибыли цепочки: {:?}", e),
        }
    }
    ...
    pub async fn calculate_chain_profit(&self, chain: Vec<String>, amount: f64) -> Result<ChainResult> {
        ...
        for (i, window) in chain.windows(2).enumerate() {
            ...
            if new_amount == 0.0 {
                ...
                #[cfg(debug_assertions)]
                info!("Недостаточная ликвидность для полной операции. Прерывание расчета цепочки. Время расчета цепочки: {:?}", chain_duration);
        ...
        #[cfg(debug_assertions)]
        info!(
            "Расчет цепочки завершен: Цепочка: {:?}, Начальная сумма: {}, Конечная сумма: {}, Процент прибыли: {:.2}%, Общее время расчета: {:?}, Общее время расчета цепочки: {:?}, Время выполнения операций: {:?}",
            chain,
    ...
    fn process_pair(&self, order_book: &OrderBookUpdate, amount: f64, _is_inverse: bool, operation: Operation) -> Result<(f64, Duration)> {
        ...
        if total_available_volume < amount {
            let mut order_book_info = String::new();
            for (i, entry) in entries.iter().enumerate().take(10) {
                order_book_info.push_str(&format!("\n  Уровень {}: Цена = {}, Объем = {}", i+1, entry.price, entry.volume));
            }
            #[cfg(debug_assertions)]
            info!("Недостаточная ликвидность для операции.\nТребуемый объем: {}, Доступный объем: {}\nСтакан ордербука:{}", amount, total_available_volume, order_book_info);
    ...
    pub async fn find_chains_with_token(&self, token: &str) -> Vec<Vec<String>> {
        ...
        #[cfg(debug_assertions)]
        info!("Обработано цепочек: {}, Найдено уникальных обрезанных цепочек: {}, Время выполнения: {:?}", 
              processed_chains, unique_trimmed_chains.len(), cycle_start.elapsed());
